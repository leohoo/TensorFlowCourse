...
# Q = 50
# W4 = tf.Variable(tf.truncated_normal([P, Q], stddev=0.1))
# b4 = tf.Variable(tf.zeros([Q]))
# L4 = tf.nn.tanh(tf.matmul(L3_drop, W4)+b4)
# L4_drop = tf.nn.dropout(L4, keep_prob)

Wo = tf.Variable(tf.truncated_normal([P, 10], stddev=0.1))
bo = tf.Variable(tf.zeros([10]))

prediction = tf.nn.softmax(tf.matmul(L3_drop, Wo)+bo)
...
with tf.Session() as sess:
    ...
    for epoch in range(100):
        ...
        print("Iter " + str(epoch) + ", Testing accuracy " + str(acc) + ", Train acc: " + str(train_acc) + ", Delta: " + str(train_acc - acc))
